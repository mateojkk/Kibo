...
export async function handleCommand(
  ...
  if (cmd.type === 'help') {
    push(
      { kind: 'separator' },
      { kind: 'info', text: '  ✦ KIBO: PRIVATE CONVERSATIONAL FINANCE ✦' },
      { kind: 'output', text: '  Talk to your wallet using clean, conversational terminal commands.' },
      { kind: 'separator' },
      { kind: 'info', text: '  PAYMENTS' },
      { kind: 'output', text: '  balance                          — view all stablecoin & SUI balances' },
      { kind: 'output', text: '  send <amount> to <name|address>  — send funds instantly (Private Toggle decides mode)' },
      { kind: 'output', text: '  claim <amount> <salt>            — unshield and claim a private payment gaslessly' },
      { kind: 'output', text: '  history                          — view recent transaction history' },
      { kind: 'separator' },
      { kind: 'info', text: '  CONTACTS' },
      { kind: 'output', text: '  contacts                         — view your address book' },
      { kind: 'output', text: '  add <name> <address>             — add a new contact' },
      { kind: 'output', text: '  remove <name>                    — remove a contact' },
      { kind: 'separator' },
      { kind: 'info', text: '  WALLET & SECURITY' },
